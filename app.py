...
from flask_login import UserMixin, login_user, LoginManager, login_required, logout_user, current_user #Manager user authentication
...
class User(db.Model, UserMixin):
    ...
    cart = db.relationship('CartItem', backref='user', lazy=True) #lazy=True is used so that cart item information is only loaded when the user tries to access it.
## Product Model
...
def add_to_card(product_id):
    ...
    if user and product:
        ...
        db.session.commit()
        return jsonify({"message": "Item added to the card successfully."})
...
def remove_from_cart(product_id):
    ...
    if cart_item:
        ...
        db.session.commit()
        return jsonify({"message": "Item removed from the card successfully."})
